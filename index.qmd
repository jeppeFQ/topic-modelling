...
## Vektorisering af tekst 

<!-----

Preprocessing

we will use Spacy for text cleaning:
    ...
    Return lemmatization form of the words
...
The LDA algorithm uses an iterative process to learn the latent topics and their associated word distributions. The basic steps of the algorithm are:

    Initialize the topic assignments for each word in each document randomly. For each document:
        Compute the topic proportions for the document based on the current topic assignments.
        For each word in the document:
        Compute the probability of each topic generating the word based on the current topic proportions and word distribution.
        Assign the word to a new topic based on these probabilities. Repeat steps 2 and 3 until convergence.
    Once the algorithm has converged, the learned topic distributions can be used to identify the most important topics in the collection and the words that are most strongly associated with each topic.
...
Gensim will go through the process of finding the best word distribution for each topic, and best topic distribution for each document.
    ...
    Input:
        Document-term matrix, a Bag-Of-Words (BOW) format.
        Number of topics
        Numper of iterations
    Output: the top words in each topic


we pass the vectorized corpus to the LDA model for both the packages gensim and sklearn.
...
Finally, we'll generate our base LDA model.

Since we have a small to medium-sized dataset, we can use LdaModel for topic modeling. However, if we have a large dataset, it is recommended to use LdaMulticore to speed up the computation.

My hypothesis is that there will be 5 Topics as there was 5 seasons.
